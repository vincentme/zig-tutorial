...
**题目1**：创建一个 `build.zig.zon` 文件，定义项目基本信息。
...
**题目3**：使用 `zig fetch` 命令获取依赖的 hash。

**要求**：
- 获取指定 URL 的依赖
- 输出 hash 值
- 将 hash 添加到 build.zig.zon
...
pub fn build(b: *std.Build) void {
    ...
    // 依赖 1
    const dep1 = b.dependency("dep1", .{
        .target = target,
        .optimize = optimize,
    });

    // 依赖 2
    const dep2 = b.dependency("dep2", .{
        .target = target,
        .optimize = optimize,
    });
    ...
    exe.root_module.addImport("dep1", dep1.module("dep1"));
    exe.root_module.addImport("dep2", dep2.module("dep2"));

    b.installArtifact(exe);
}
```

**题目2**：创建一个库项目，并配置为可被其他项目依赖。
...
pub fn build(b: *std.Build) void {
    ...
    // 导出库模块
    const lib_module = b.addModule("mylib", lib_mod);
    ...
    exe_mod.addImport("mylib", lib_module);
...
> 💡 **章节过渡**：从理论到实战应用
> 
> 在前面的章节中，我们系统学习了 Zig 的核心特性和高级功能：
> - 基础语法、控制流、函数和错误处理（第3-6章）
> - 构建系统和标准库（第7-8章）
> - 编译期计算、泛型、接口和内存管理（第9-12章）
> - 并发编程、指针、互操作和包管理（第13-17章）
> 
> 现在，我们将进入实战应用阶段，通过真实案例巩固所学知识：
> - 测试与基准测试（第18章）
> - CLI工具开发（第19章）
> - HTTP服务器开发（第20章）
> - 内存池实现（第21章）
...
> **为什么需要实战应用？**
> 
> 1. **知识整合**：将零散的知识点整合为完整的解决方案
> 2. **最佳实践**：学习真实项目中的设计模式和编码规范
> 3. **问题解决**：培养分析和解决实际问题的能力
> 
> **学习建议**：
> - 确保你已经掌握了前面章节的核心概念
> - 动手实践每个案例，不要只看不练
