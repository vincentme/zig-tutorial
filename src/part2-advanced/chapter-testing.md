...
1. **先测核心行为** — 也就是这个函数最主要的承诺是什么。例如 `divide(a, b)`：正常除法能否返回结果？除数为零时是否返回预期错误？
2. **再测边界条件** — 空输入、最小值/最大值、长度为 0 或 1 的切片、极小/极大容量、容器空/满状态。
3. **再测失败路径** — 尤其是分配失败、输入无效、文件不存在、解析失败、资源初始化中途失败等场景。
4. **最后再补回归测试** — 如果修过一个 bug，就应该尽量为那个 bug 增加一个测试，让它以后不会悄悄回来。
...
一些资料中可能出现 `std.testing.refAllDecls(...)`。  
...
### 1. 关注一个明确行为
不要在一个测试里同时验证十件事。否则一旦失败，很难快速定位原因。

> **反面**：一个测试塞太多断言，失败时难以定位问题。
...
### 4. 对失败路径同样重视
失败路径常常更值得测，尤其在 Zig 中。

> **反面**：只测快乐路径是最常见的问题。

### 5. 不依赖隐式全局状态，理清资源所有权
如果测试必须依赖复杂外部状态，通常说明设计可以继续改进。测试里说不清楚资源所有权，往往说明接口设计也还不够清楚。
