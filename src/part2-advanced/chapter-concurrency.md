...
fn consumer() void {
    ...
    std.debug.print("resource is ready\n", .{});
}
```
...
> 💡 一个实用经验：
>
> 如果你在写原子代码时已经开始怀疑“这到底是否还容易验证”，那通常就该重新评估，看看是不是应该回到锁。
...
你可能已经在博客、issue、标准库源码或开发版讨论中看到过与异步 I/O 相关的方向性接口。
...
4. 真要落地具体 API，必须以你本地版本和源码为准

因此，本章不把某个开发版里的 I/O 并发接口当作“你现在必须背下来的主线知识”。
...
在你还没完全掌握线程、锁、条件变量之前，过早上复杂 lock-free 结构、复杂原子协议，通常只会让代码更难验证。

## 本章小结

本章最重要的目标，不是让你一次掌握所有并发技巧，而是建立这几个核心判断：
...
如果你已经能看清这些边界，那么在后续阅读测试、构建、网络和实战章节时，就更不容易把“并发写法”误当成“只是语法技巧”。