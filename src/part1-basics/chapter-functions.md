...
在深入学习函数语法之前，理解Zig的函数设计哲学至关重要。

> 📜 **Zig Zen 原则关联**
> 
> 本章内容体现了以下 Zig Zen 原则：
> - **"Communicate intent precisely."**（精确表达意图）  
>   Zig 函数要求显式声明错误类型（`!T`），显式传递分配器参数，明确表达函数的行为和副作用。调用者一眼就能看出函数可能返回什么错误，需要什么资源。
> - **"Only one obvious way to do things."**（只有一种显而易见的方式来做事情）  
>   Zig 只有一种错误处理方式：错误联合类型。没有异常、没有 Result 类型、没有 option 类型混用。统一的错误处理模型降低了学习成本。
> - **"Edge cases matter."**（边界情况很重要）  
>   Zig 强制处理所有可能的错误情况。使用 `!T` 类型时，调用者必须显式处理错误，不能忽略。这确保边界情况得到妥善处理。
...
**题目1**：编写一个函数，计算两个整数的最大公约数（GCD）。

**要求**：
- 使用欧几里得算法
- 函数签名为 `fn gcd(a: u32, b: u32) u32`
- 测试多组数据
...
**题目3**：编写一个递归函数，计算斐波那契数列的第 n 项。

**要求**：
- 函数签名为 `fn fib(n: u32) u32`
- 处理边界情况（n = 0, n = 1）
...
**题目1**：实现一个高阶函数，接受一个函数和切片，对每个元素应用该函数。

**要求**：
- 函数签名为 `fn map(comptime T: type, slice: []T, func: fn (T) T) void`
- 修改原切片

**参考答案**：
```zig
fn map(comptime T: type, slice: []T, func: fn (T) T) void {
    for (slice) |*item| {
        item.* = func(item.*);
    }
}
...
**题目2**：实现一个函数，使用指针参数修改多个返回值。

**要求**：
- 函数签名为 `fn divide(a: i32, b: i32, quotient: *i32, remainder: *i32) void`
- 同时返回商和余数

**参考答案**：
```zig
fn divide(a: i32, b: i32, quotient: *i32, remainder: *i32) void {
...
**参考答案**：
```zig
fn compose(comptime F: type, comptime G: type) type {
    return struct {
        f: F,
        g: G,
        
        fn call(self: @This(), x: anytype) @TypeOf(self.f(self.g(x))) {
...
pub fn main(init: std.process.Init.Minimal) void {
    ...
    const composed = compose(@TypeOf(double), @TypeOf(addOne)){
        .f = double,
        .g = addOne,
    };
    
    const result = composed.call(5);
    std.debug.print("结果：{}\n", .{result});
}
```
...
> 💡 **章节过渡**：从函数到错误处理
> 
> 在[函数定义与调用](chapter-functions.md)中，我们学习了函数的定义与调用，掌握了参数传递、返回值、闭包等核心概念。
> 现在，我们将学习错误处理，了解 Zig 如何实现安全、显式的错误管理。
> 
> **为什么函数是错误处理的基础？**
> 
> 1. **错误类型**：函数签名中的 `!T` 表示可能返回错误
> 2. **错误传播**：`try` 和 `catch` 关键字用于处理函数返回的错误
> 3. **资源管理**：`errdefer` 用于错误发生时的资源清理
> 
> **学习建议**：
> - 理解函数返回值的类型系统
> - 掌握 `try`、`catch` 的基本用法
