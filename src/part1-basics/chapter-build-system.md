...
`src/main.zig` 通常是可执行程序入口所在的位置。

### `src/root.zig`

`src/root.zig` 通常作为库的根模块入口使用。

很多最小项目在前期主要关注可执行入口，因此这一文件可能暂时不会频繁使用。
...
在基础阶段，`target` 和 `optimize` 不展开到全部细节，但它们是最常见的构建参数。
...
本章不展开各种优化模式之间的完整差异，这里主要关注它作为常见构建参数的作用。
...
本章到这里已经覆盖了基础部分所需的构建系统认识。下面对更复杂的主题做边界说明。
...
这些主题都很重要，但更适合放到高级部分系统展开。本章的重点是建立项目级构建入口的基础认识。
...
这些内容并不是次要主题，只是它们建立在“项目工作流”这一基础之上。先把基础闭环走通，再进入高级配置，会更清楚每个构建部分分别在解决什么问题。
