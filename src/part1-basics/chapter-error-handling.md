...
许多语言把失败路径放在异常系统中，函数签名不直接体现失败可能，控制流可能在运行时离开当前路径。Zig 采用了不同的设计：**把失败纳入类型系统**，通过类型和控制流清楚表达操作是否可能失败、失败时返回什么、错误应当在何处处理。这一设计使得接口更精确、资源管理更可靠，也更适合系统编程场景。语言设计层面的更多讨论见[认识 Zig](chapter-introduction.md)。
...
3. **关于错误传播与处理策略的选择**（何时用 `try`、何时用 `catch`），见前面的 [`try`：传播错误](#try传播错误)和 [`catch`：在当前层处理错误](#catch在当前层处理错误)小节。

4. **关于 `catch unreachable` 的使用注意**，见前面的 [catch unreachable](#catch在当前层处理错误)小节。
