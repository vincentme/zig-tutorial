...
`noreturn` 表示控制流不会继续返回到当前位置。
- 典型例子包括：`while (true) {}`、`unreachable`、`std.process.exit()`；会直接结束当前控制流的表达式（如 `return`）的类型也为 `noreturn`。
...
> **深入学习**：字符串格式化（如 `{s}`, `{c}`, `{u}` 等格式说明符）的详细用法请参考[常用标准库模块详解](../part2-advanced/chapter-standard-library-detail.md#stdfmt)中的相关内容。
...
读完这一章后，建议你先牢牢记住下面这些主线，而不必急着把所有细节一次记完：
