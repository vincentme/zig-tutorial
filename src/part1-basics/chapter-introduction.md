...
- **编译期能力**：Zig 的 `comptime` 不是单纯"做模板"或"写宏"，而是让开发者用同一套语言语法，把一部分逻辑提前到编译期完成，这也是 Zig 在泛型、元编程、编译期检查等方面很有辨识度的原因之一。
- **跨平台与 C 互操作**：Zig 对交叉编译和 C 互操作的支持非常强，这让它在替代部分 C/C++ 基础设施、构建工具、CLI 工具或平台胶水层时非常有竞争力。
...
Zig 和 C 都强调底层控制能力，但 Zig 提供了更统一的工具链、更清晰的错误处理模型和更强的编译期能力，同时避免了预处理器式的技巧。

### 和 C++ 的关系

Zig 有意减少语言复杂度，不追求"什么都能抽象"，而更强调规则少、特性交互直接、元编程能力统一、可读性优先。

### 和 Rust 的关系

两者都在改进系统编程体验，但路径不同：Rust 更强调通过编译期约束提供强安全保证，Zig 更强调显式控制、简单规则和更直接的底层协作能力。

### 和 Go 的关系

Zig 没有垃圾回收，资源管理和失败路径都需要显式处理，换来的是更可预测的性能、更低的运行时成本和更直接的底层控制能力。
